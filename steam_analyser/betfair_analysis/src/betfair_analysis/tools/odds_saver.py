...
class OddsSaver:
    """
    Saves match odds for a given team using a particular betfair data file.
    ...
    def save_odds(self, data_file: str, output_pickle: str):
        ...
        for file in bfd.Files([data_file]):
            for market in file:
                ...
                if self.home_only and not market.event_name.startswith(f"{self.team} v "):
                    continue
                if not self.home_only and not market.event_name.endswith(f" v {self.team}"):
                    continue
